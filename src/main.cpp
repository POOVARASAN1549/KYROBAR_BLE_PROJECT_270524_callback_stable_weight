...
void setup() {
  ...
  stableData = 0; // Initialize stableData
}
...
void loop() {
  if (scale.is_ready()) {
    ...
    // Apply 2-gram tolerance
    if (abs(loadCellData - stableData) > TOLERANCE) {
      stableData = loadCellData; // Update stableData only if the change exceeds the tolerance

      // Convert load cell data to string
      char dataString[20];
      sprintf(dataString, "%ld", stableData);
